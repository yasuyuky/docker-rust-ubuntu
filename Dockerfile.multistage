...
RUN --mount=type=cache,target=/usr/local/cargo/registry \
    ...
    set -eux; \
    echo "Building wild-linker for TARGETARCH=$TARGETARCH"; \
    if [ "$TARGETARCH" = "arm" ]; then \
    printf "#!/bin/sh\nexec echo 'wild is skipped on TARGETARCH=%s'" "$TARGETARCH" > /usr/local/bin/wild; \
...
# ========================================
# Stage 3: Final image with tools
# ========================================
FROM base AS final
ARG TARGETARCH
